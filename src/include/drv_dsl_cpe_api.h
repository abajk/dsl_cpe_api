...
/** \defgroup DRV_DSL_CPE_API MaxLinear DSL CPE API Driver
    Lists the entire modules to the DSL CPE_API.
*/

/** \defgroup DRV_MEI MaxLinear MEI Driver
...
/**
   Reports if and which kind of bonding was actually selected in G.HS
*/
typedef enum
{
   /**
   Non-bonded mode
   */
   DSL_BONDING_USED_NO_BOND = 0,
   /**
   PAF Bonding
   */
   DSL_BONDING_USED_PAF = 1,
   /**
   ATM Bonding (IMA+)
   */
   DSL_BONDING_USED_IMAP = 2
} DSL_BondingMode_t;
...
typedef struct
{
   ...
   - Indicates the major feature set implementation for firmware (please refer
     to firmware documentation and/or contact MaxLinear for detailed informations).
   ...
   - minor version (minor): logical number, incremented for minor changes in
     versions
   - build step (build): logical number, incremented for each new version build
   - optional release indictor (rel): specifies different MaxLinear internal
     indications */
...
typedef enum
{
   ...
   \note This command will terminate the autoboot handling
         and terminate line (stop FW). */
