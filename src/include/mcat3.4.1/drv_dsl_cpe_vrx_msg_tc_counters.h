...
#define ACK_TC_StatusGet_NO_BOND 0x0
#define ACK_TC_StatusGet_PAF 0x1
#define ACK_TC_StatusGet_IMAP 0x2
#define EVT_TC_StatusGet_EFM_TC 0x1
#define EVT_TC_StatusGet_ATM_TC 0x2
#define EVT_TC_StatusGet_NO_BOND 0x0
#define EVT_TC_StatusGet_PAF 0x1
#define EVT_TC_StatusGet_IMAP 0x2
...
#define ACK_VersionInfoGet_HW_VER21 0x2
...
/**
   ...
   bearer channel 0 (BC0) as defined in G.997.1 (Section 7.2.5.1. of [7]).The
...
/**
   ...
   as requested by CMD_PTM_BC0_StatsNE_Get. (Section 7.2.5.1. of [7])
*/
...
/**
   ...
   7.2.5.1. of G.997.1 [7]).Why writing these counters? For "automoding",
...
/**
   ...
   for BC0 of an ATM-TC link as defined in Section 7.2.4. of G.997.1 [7].
*/
...
/**
   ...
   for BC0 of an ATM-TC link as defined in Section 7.2.4.2 of G.997.1 [7].
*/
...
/**
   ...
   ATM-TC link (Section 7.2.4. of G.997.1 [7]).Why writing counters? The ADSL FW
   of the VRX destroys the performance counters at the beginning of a re-
...
/**
   ...
   writing performance counters" see Page 692.
*/
...
/**
   ...
   numbering scheme has a long and a short form (Table 380).
*/
...
/** Message ID for CMD_Bonding_HS_Control */
#define CMD_BONDING_HS_CONTROL 0x1148

/**
   The message is used to enable bonding (PAF and/or ATM bonding). For ATM
   bonding only the G.HS part is done in FW and controlled by this message. The
   bonding functionality itself is implemented in the Host SW.
*/
typedef struct CMD_Bonding_HS_Control CMD_Bonding_HS_Control_t;

/** Message ID for ACK_Bonding_HS_Control */
#define ACK_BONDING_HS_CONTROL 0x1148

/**
   The message acknowledges CMD_Bonding_HS_Control.
*/
typedef struct ACK_Bonding_HS_Control ACK_Bonding_HS_Control_t;

/** Message ID for CMD_Bonding_HS_ControlGet */
#define CMD_BONDING_HS_CONTROLGET 0x1108

/**
   The message reads back settings for bonding configured with
   CMD_Bonding_HS_Control.
*/
typedef struct CMD_Bonding_HS_ControlGet CMD_Bonding_HS_ControlGet_t;

/** Message ID for ACK_Bonding_HS_ControlGet */
#define ACK_BONDING_HS_CONTROLGET 0x1108

/**
   The message acknowledges CMD_Bonding_HS_ControlGet providing bonding
   parameters used in handshake as configured with CMD_Bonding_HS_Control.
*/
typedef struct ACK_Bonding_HS_ControlGet ACK_Bonding_HS_ControlGet_t;
...
/** Message ID for CMD_IMAP_HS_StatusGet */
#define CMD_IMAP_HS_STATUSGET 0x8303

/**
   The message reads G.Hs Handshake status parameters for ATM bonding.
*/
typedef struct CMD_IMAP_HS_StatusGet CMD_IMAP_HS_StatusGet_t;

/** Message ID for ACK_IMAP_HS_StatusGet */
#define ACK_IMAP_HS_STATUSGET 0x8303

/**
   The message reports G.Hs Handshake status parameters for ATM bonding.
*/
typedef struct ACK_IMAP_HS_StatusGet ACK_IMAP_HS_StatusGet_t;

/** Message ID for CMD_IMAP_Control */
#define CMD_IMAP_CONTROL 0x0441

/**
   The message is used to write the SW detected ATM bonding mode (on or off) to be used in the next initialization.
   FW will store it in persistent memory (to survive a FW download). It is used during the next initialization: If ATMTC is selected in G.Hs phase, then after G.Hs the IMA+ selection is indicated to the host with EVT_TC_StatusGet,
   where it is used finally to activate/deactivate the IMA+ Bonding driver.
   Note: In particular, if IMA+ usage detection is done on 1 link only, the other link has to be forced to use the same
   detected value by this FW message
*/
typedef struct CMD_IMAP_Control CMD_IMAP_Control_t;

/** Message ID for ACK_IMAP_Control */
#define ACK_IMAP_CONTROL 0x0441

/**
   The message acknowledges CMD_IMAP_Control.
*/
typedef struct ACK_IMAP_Control ACK_IMAP_Control_t;
...
struct ACK_ADSL_FeatureMapGet
{
   ...
   /** Feature-Bit1 of Word 1: ATM Bonding (IMA+) */
   DSL_uint16_t W1F01 : 1;
   /** Feature-Bit0 of Word 1: PTM Bonding (PAF), G.hs based */
...
struct CMD_Bonding_HS_Control
{
   ...
   /** ATM Bonding Control */
   DSL_uint16_t IMAP_Enable : 1;
   /** PAF Bonding: PCS Control Register (Register 3.61, Bit 0) */
...
/**
   ...
   CMD_Bonding_HS_Control.
*/
struct CMD_Bonding_HS_ControlGet
{
#if DSL_BYTE_ORDER == DSL_BIG_ENDIAN
   /** Index */
   DSL_uint16_t Index;
...
/**
   ...
   parameters used in handshake as configured with CMD_Bonding_HS_Control.
*/
struct ACK_Bonding_HS_ControlGet
{
#if DSL_BYTE_ORDER == DSL_BIG_ENDIAN
   /** Index */
   DSL_uint16_t Index;
...
struct ACK_IMAP_HS_StatusGet
{
   ...
   DSL_uint16_t Res0 : 15;
   /** IMA+ FE Support */
   DSL_uint16_t IMAP_FE : 1;
   /** Max Differential Delay DS */
   DSL_uint16_t MaxDiffDelay;
#else
   /** Index */
   DSL_uint16_t Index;
   /** Length */
